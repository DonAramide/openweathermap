package com.example.aramydeopenweathermap.implementation;
...
import com.example.aramydeopenweathermap.implementation.callback.ThreeHourForecastCallback;
...
import com.example.aramydeopenweathermap.network.OpenWeatherMapClient;
import com.example.aramydeopenweathermap.network.OpenWeatherMapService;
...
//    START
    ...
    private void handleSevenDaysForecastResponse(Response<SevendaysForecast> response, SevenDaysForecastCallback callback){
        if (response.code() == HttpURLConnection.HTTP_OK){
            callback.onSuccess(response.body());
        }
        else if (response.code() == HttpURLConnection.HTTP_FORBIDDEN || response.code() == HttpURLConnection.HTTP_UNAUTHORIZED){
    ...
    public void getSevenDays( final SevenDaysForecastCallback callback){
        options.put(exclude, "minutely");
        options.put(QUERY, "NIGERIA");
        openWeatherMapService.getSevenDays(options)
                .enqueue(new Callback<SevendaysForecast>() {
                    @Override
                    public void onResponse(@NonNull Call<SevendaysForecast> call, @NonNull Response<SevendaysForecast> response) {
                        handleSevenDaysForecastResponse(response, callback);
                    }

                    @Override
                    public void onFailure(@NonNull Call<SevendaysForecast> call, @NonNull Throwable throwable) {
                        callback.onFailure(throwable);
                    }
                });
